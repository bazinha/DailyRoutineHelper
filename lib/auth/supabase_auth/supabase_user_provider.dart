...
class DailyRoutineHelperSupabaseUser extends BaseAuthUser {
  DailyRoutineHelperSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> dailyRoutineHelperSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DailyRoutineHelperSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
